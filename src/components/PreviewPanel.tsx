...
export default function PreviewPanel() {
  const { previewOpen, setPreviewOpen, generatedMarkdown, equippedSlots, exportToClipboard } =
    useBuildStore()

  const equippedItems = Object.values(equippedSlots).filter(Boolean) as NonNullable<
    (typeof equippedSlots)[keyof typeof equippedSlots]
  >[]
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span
            className={`text-amber-100/60 transition-transform ${previewOpen ? 'rotate-90' : ''}`}
          >
            ▶
          </span>
